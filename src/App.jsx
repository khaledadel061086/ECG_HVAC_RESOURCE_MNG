...
function OrgChart({engineers, projects}) {
  ...
  // ── Apply branch filter ─────────────────────────────────────────────────────
  const filteredEngineers = filterBranch==="All"
    ? engineers
    : engineers.filter(e => e.branch===filterBranch);

  const filteredProjects = filterBranch==="All"
    ? projects
    : projects.filter(p => {
        const ldr = engineers.find(e => e.id===p.leaderId);
        return ldr?.branch===filterBranch;
      });

  const leaders = filteredEngineers.filter(e => e.option==="Team Leader");
  ...
  const hierarchyNodes = useMemo(()=>{
    ...
    leaders.forEach((leader,li)=>{
      ...
      const ldrMembers=[...new Set(
        filteredProjects.filter(p=>p.leaderId===leader.id).flatMap(p=>p.members.map(m=>m.engId))
      )];
  ...
  const networkNodes = useMemo(()=>{
    ...
    mems.forEach((m,i)=>{
      ...
      const primaryLeader=leaders.find(l=>filteredProjects.some(p=>p.leaderId===l.id&&p.members.some(x=>x.engId===m.id)));
      nodes.push({
