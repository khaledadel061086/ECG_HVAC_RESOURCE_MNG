  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data, error: dbErr } = await supabase
        ...
        .eq("username", u)
        .single();
      if(dbErr || !data){ setError("Invalid username or password."); setLoading(false); return; }
      if(data.pass_hash !== hash){ setError("Invalid username or password."); setLoading(false); return; }
      // Update last_login
      await supabase.from("app_users").update({ last_login: new Date().toISOString() }).eq("username", u);
...
export default function App() {
  ...
  // On mount: seed admin account + check session
  useEffect(()=>{
    (async()=>{
      // Seed / verify admin password hash in Supabase
      try {
        const correctHash = await sha256(ADMIN_PASSWORD);
        const {data:adminRow} = await supabase
          .from("app_users").select("pass_hash").eq("username",ADMIN_USERNAME).single();
        if(adminRow && adminRow.pass_hash !== correctHash){
          await supabase.from("app_users")
            .update({pass_hash: correctHash, display_name:"Khaled Adel", is_admin:true})
            .eq("username", ADMIN_USERNAME);
        }
        if(!adminRow){
          await supabase.from("app_users")
            .upsert({username:ADMIN_USERNAME, pass_hash:correctHash, is_admin:true, display_name:"Khaled Adel"});
        }
  ...
  const handleLogin = (user) => {
    setAuthUser(user);
    sessionStorage.setItem("hvacflow_session", JSON.stringify(user));
  };
  const handleLogout = () => {
    setAuthUser(null);
    sessionStorage.removeItem("hvacflow_session");
  };
